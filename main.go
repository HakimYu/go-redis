...
func usage() {
	...
	fmt.Println("     - set (key) (value) (expire_time): 设置一个键值对，expire_time 为过期时间（单位：分钟）")
	fmt.Println("     - get (key): 获取指定键的值，如果键不存在，将返回错误信息")
	fmt.Println("     - setnx (key) (value) (expire_time): 仅在键不存在时设置键值对，防止覆盖已有的键，expire_time 为过期时间（单位：分钟）")
